<template>
    <v-row no-gutters>
        <v-col cols="12" md="12">
            ...
            <v-form @submit.prevent="addProblems()" ref="problemForm">
                <v-row justify="space-between" class="align-center mb-3">
                    <v-col cols="12" md="6" lg="6">
                        <div class="d-flex">
                            <v-label class="font-weight-medium text-capitalize required mb-4 mr-2">Add new problem type</v-label>
                            <v-text-field
                                required
                                name="problemType"
                                v-model="problemType"
            ...
            <div id="infinite-list">
                <v-form @submit.prevent="" ref="problemEditForm"  class="w-50">
                    <EasyDataTable
                        ...
                        :loading="isLoading"
                    >
                        <template #item-problem_type="{ problem_name, id }">
...
const dialogTitle = ref('Are you sure you want to delete this Problem Type ?');
const dialogText = ref('This will delete this Problem Type permanently, you can not undo this action.');
...
const headers = ref([
    { text: 'Problem Type', value: 'problem_type' },
...
async function updateProblem(id) {
    ...
    if (valid) {
        isEdit.value = true;
        const requestBody = {
            problem_name: editproblemType.value
        };
        baseURlApi
            .post(`settings/problem-type/update/${id}`, requestBody)
            .then((res) => {
                isEdit.value = false;
...
function editProblem(id) {
    baseURlApi
        .get(`settings/problem-type/edit/${id}`)
        .then((res) => {
            editID.value = id;
...
async function addProblems() {
    ...
    if (valid) {
        issubmit.value = true;
        const requestBody = {
            problem_name: problemType.value
        };
        baseURlApi
            ...
            .then((res) => {
                ...
                problemForm.value?.resetValidation();
                getProblems();
                dialog.value = false;
            })
            .catch((error) => {
...
function confirmClick() {
    baseURlApi
        .delete(`settings/problem-type/delete/${deleteId.value}`)
        .then((res) => {
            deleteDialog.value?.close();
            getProblems();
            showSnackbar.value = true
