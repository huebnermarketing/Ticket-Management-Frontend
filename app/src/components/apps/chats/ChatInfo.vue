...
const totalAttachment = computed(() => {
    return uniq(flatten(props.chatDetail?.chatHistory.map((item) => item.attachment))).length;
});

const totalMedia = computed(() => {
    return uniq(flatten(props.chatDetail?.chatHistory.map((item) => (item?.type === 'img' ? item.msg : null)))).length - 1;
});
</script>
<template>
